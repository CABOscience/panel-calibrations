...
# Panel ID
panel_id <- '99AA10-0120-3673'
calibration_date <- '2020-02-04'
...
# Plot the data
plot(calib_raw, type = 'l')

# remove six really high values >.997
# Not needed for this calibration because no outliers
...
calib_raw_wide <- calib_raw %>%
  ...
  mutate(SN = 'CSRS-99-010') %>%
  as.data.frame()
calib_raw_spectra <- as_spectra(calib_raw_wide, name_idx = ncol(calib_raw_wide))
...
calib_match <- calib_raw_spectra

# Resample to 1-nm and smooth
wvls <- 250:2500
calib_smooth_0.1 <- spectrolab::resample(calib_match, new_bands = wvls, spar = 0.1)
calib_smooth_0.2 <- spectrolab::resample(calib_match, new_bands = wvls, spar = 0.2)
calib_smooth_0.3 <- spectrolab::resample(calib_match, new_bands = wvls, spar = 0.3)
calib_smooth_0.4 <- spectrolab::resample(calib_match, new_bands = wvls, spar = 0.4)
calib_smooth_0.5 <- spectrolab::resample(calib_match, new_bands = wvls, spar = 0.5)
calib_smooth_0.6 <- spectrolab::resample(calib_match, new_bands = wvls, spar = 0.6)
calib_smooth_0.7 <- spectrolab::resample(calib_match, new_bands = wvls, spar = 0.7)
calib_smooth_0.8 <- spectrolab::resample(calib_match, new_bands = wvls, spar = 0.8)
